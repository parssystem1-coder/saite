...
## 🔑 «رمز نادرست» با اینکه رمز درست است

### نشانه

رمز مدیر را عوض کرده‌اید، مطمئنید درست تایپ می‌کنید، اما ورود
همیشه می‌گوید «نام کاربری یا رمز عبور نادرست است».
...
اگر گفت «رمز عبور: هش‌شده با scrypt» ولی ورود کار نمی‌کند، ادامه
را بخوانید.

### علت رایج: کاراکتر `$` در فایل env

Next.js مقادیر `.env.local` را از یک تابع بسط متغیر رد می‌کند.
اگر مقدار شما `$` داشته باشد، آن را **نام متغیر** می‌بیند و با
رشتهٔ خالی جایگزین می‌کند:

```
ADMIN_PASSWORD=my$ecret          → myecret   ❌
ADMIN_PASSWORD=pass$123word      → password  ❌
```
...
```bash
# ۱. با \ فرار دهید
ADMIN_PASSWORD=my\$ecret

# ۲. رمزی بدون $ انتخاب کنید (ساده‌ترین)
ADMIN_PASSWORD=my-secret-1404

# ۳. هش کنید — فرمت هش عمداً هیچ $ ندارد
npm run admin:hash-password
```

> هش‌های این پروژه با `.` جدا می‌شوند نه `$`، دقیقاً به همین
> دلیل. اگر جایی هش قدیمی با `$` دارید، دوباره بسازیدش.

### علت دیگر: سرور را ری‌استارت نکرده‌اید

متغیرهای محیطی فقط در زمان **شروع** خوانده می‌شوند. پس از تغییر
`.env.local`:

```bash
# سرور را ببندید (Ctrl+C) و دوباره اجرا کنید
npm run dev
```

---

## 🔒 قفل شدم و نمی‌توانم وارد شوم

### پس از تلاش‌های ناموفق (خطای ۴۲۹)

شمارنده روی دیسک است و با ری‌استارت پاک نمی‌شود — این عمدی است.
برای پاک‌کردن دستی:

```bash
rm .next/cache/saite-rate-limit.json
```

### کد دومرحله‌ای را از دست داده‌ام

اگر گوشی را گم کرده‌اید و کلید پشتیبان هم ندارید، خط زیر را از
`.env.local` حذف کنید و سرور را دوباره اجرا کنید:

```
ADMIN_TOTP_SECRET=...
```

ورود دومرحله‌ای غیرفعال می‌شود و با نام کاربری و رمز وارد
می‌شوید. سپس با `npm run admin:totp` کلید تازه بسازید.

> این راه فقط چون فایل روی سیستم خودتان است ممکن است. پس از
> انتقال به دیتابیس، کدهای پشتیبان یک‌بارمصرف جایش را می‌گیرند.
