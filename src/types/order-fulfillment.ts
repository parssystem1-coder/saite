...
export interface OrderFulfillment { orderId:string; recipient:RecipientAddress; items:FulfillmentItem[]; packages:ShipmentPackage[]; returns:ReturnRequest[]; status:FulfillmentOrderStatus; orderTotal:number; declaredTotal:number; internalNote?:string; updatedAt:string; updatedBy:string; }
