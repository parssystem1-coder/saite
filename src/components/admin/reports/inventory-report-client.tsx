    ...
    <div className="space-y-6">
      ...
      {adjusting && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-overlay p-4" role="dialog" aria-modal="true" aria-label="اصلاح موجودی">
        <div className="surface-3d w-full max-w-md rounded-2xl p-5">
          ...
          <label className="mb-3 block text-sm">تغییر موجودی<input autoFocus value={deltaText} onChange={(e) => setDeltaText(e.target.value)} inputMode="numeric" placeholder="مثلاً ۱۰ یا ‎-۲" className="mt-1 w-full rounded-lg border border-border bg-surface-2 p-2" /></label>
          <label className="mb-3 block text-sm">دلیل<select value={reason} onChange={(e) => setReason(e.target.value)} className="mt-1 w-full rounded-lg border border-border bg-surface-2 p-2"><option value="receipt">ورود کالا</option><option value="correction">اصلاح دستی</option><option value="damaged">آسیب‌دیدگی</option><option value="returned">بازگشت کالا</option><option value="stocktake">انبارگردانی</option></select></label>
          <label className="mb-4 block text-sm">یادداشت <span className="text-muted-foreground">(اختیاری)</span><textarea value={note} onChange={(e) => setNote(e.target.value)} maxLength={500} className="mt-1 min-h-20 w-full rounded-lg border border-border bg-surface-2 p-2" /></label>
