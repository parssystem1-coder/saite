...
import './product-editor-table-emoji.css'
...
export default function ProductEditor({ initialValue, onSave, onPublish }: ProductEditorProps) {
  const defaultAdapter = useMemo(() => createMockProductEditorAdapter(), [])
  ...
  const uploadImage = async (file: File) => {
    try {
      ...
      const res = await defaultAdapter.uploadImage(file)
      return res.url
    }
  }

  const addCustomEmoji = async (emoji: string) => {
    try {
      ...
      if (!response.ok) throw new Error('ذخیره ایموجی ناموفق بود')
      const data = (await response.json()) as { emojis: string[] }
      setCustomEmojis(data.emojis)
    } catch {
      const emojis = await defaultAdapter.addEmoji(emoji)
  ...
  const badges = {
    base: 0,
    commerce: draft.priceToman === '' || draft.stock === '' ? 1 : 0,
    specs: attributes.filter((attribute) => !attribute.value).length,
    media: Math.max(0, 3 - images.length) + images.filter((image) => !image.alt).length,
  ...
  const save = async (publish = false) => {
    ...
    try {
      if (publish) {
        if (onPublish) await onPublish(state)
        else await defaultAdapter.publish(state)
      } else {
        if (onSave) await onSave(state)
        else await defaultAdapter.saveDraft(state)
      }
    ...
    <div dir="rtl" className="min-h-screen bg-[hsl(var(--surface-2))] pb-24 text-[hsl(var(--foreground))]">
      <header className="sticky top-[68px] z-[30] border-b border-[hsl(var(--border))] bg-[hsl(var(--surface-1)/.96)] backdrop-blur">
        <div className="mx-auto flex max-w-[1540px] items-center gap-4 px-4 py-3 lg:px-8">
          <b>Saite Admin</b>
          <div className="h-6 w-px bg-[hsl(var(--border))]" aria-hidden="true" />
          <div className="min-w-0 flex-1">
            <h1 className="truncate text-base font-bold">{draft.name || 'افزودن محصول جدید'}</h1>
            <p className="text-[11px] text-[hsl(var(--muted-foreground))]">محصول تک‌کاره، لیزری، تک‌رنگ</p>
          </div>
      ...
      <div className="mx-auto grid max-w-[1540px] gap-6 px-4 py-6 lg:grid-cols-[minmax(0,1fr)_300px] lg:px-8">
        <main className="min-w-0">
          <ProductTabs activeTab={activeTab} onChange={setActiveTab} badges={badges} />
          {activeTab === 'base' && <BasePanel draft={draft} set={set} />}
          {activeTab === 'commerce' && <CommercePanel draft={draft} set={set} />}
          {activeTab === 'specs' && <SpecsPanel attributes={attributes} onChange={setAttributes} />}
          {activeTab === 'media' && <MediaPanel images={images} onChange={setImages} />}
          {activeTab === 'content' && (
            <ContentPanel
              draft={draft}
              set={set}
        ...
        <ProductSidebar
          ...
          schemaCount={draft.activeSchemas.length}
          status={status}
          onStatusChange={setStatus}
        />
      </div>

      <footer className="fixed bottom-0 left-0 right-[52px] z-[30] border-t border-[hsl(var(--border))] bg-[hsl(var(--surface-1)/.97)] backdrop-blur lg:right-[60px]">
        <div className="mx-auto flex max-w-[1540px] items-center gap-2 px-4 py-3 lg:px-8">
          <span className="text-xs text-[hsl(var(--muted-foreground))]">
            {score < 70
              ? `${Object.values(checks).filter((value) => !value).length} مورد تا انتشار باقی مانده`
              : 'آماده انتشار'}
          </span>
          ...
          <button
            ...
            disabled={saving}
            onClick={() => save(false)}
            className="rounded-md border border-[hsl(var(--border))] bg-[hsl(var(--surface-3))] px-4 py-2 text-xs"
          >
            ذخیره پیش‌نویس
          </button>
          <button
            type="button"
            disabled={saving || score < 70}
            onClick={() => save(true)}
            className="rounded-md bg-[hsl(var(--primary))] px-4 py-2 text-xs font-bold text-[hsl(var(--primary-foreground))]"
          >
            انتشار محصول
          </button>
        </div>
