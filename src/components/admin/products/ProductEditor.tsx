...
export default function ProductEditor({ initialValue, onSave, onPublish }: ProductEditorProps) {
  ...
  return <div dir="rtl" className="min-h-screen bg-[hsl(var(--background))] pb-24 text-[hsl(var(--foreground))]">
    ...
    <footer className="fixed bottom-0 right-0 left-0 z-[80] border-t border-[hsl(var(--border))] bg-[hsl(var(--surface-1)/.97)]"><div className="mx-auto flex max-w-[1540px] items-center gap-2 px-4 py-3 lg:px-8"><span className="text-xs text-[hsl(var(--muted-foreground))]">{score < 70 ? `${Object.values(checks).filter(value => !value).length} مورد تا انتشار باقی مانده` : 'آماده انتشار'}</span><span className="flex-1" /><button type="button" disabled={saving} onClick={() => save(false)} className="rounded-md border border-[hsl(var(--border))] bg-[hsl(var(--surface-3))] px-4 py-2 text-xs">ذخیره پیش‌نویس</button><button type="button" disabled={saving || score < 70} onClick={() => save(true)} className="rounded-md bg-[hsl(var(--primary))] px-4 py-2 text-xs font-bold text-[hsl(var(--primary-foreground))]">انتشار محصول</button></div></footer>
  </div>;
}
